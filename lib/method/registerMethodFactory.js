...
const doubleSha = require('@dashevo/dpp/lib/util/hash');
const { hash: createMultiHash} = require('@dashevo/dpp/lib/util/multihashDoubleSHA256');
...
function registerMethodFactory(dapiClient, dpp, wallet, dataContract) {
  ...
  async function registerMethod(name, user, records) {
    ...
    const nameHash = createMultiHash(
      doubleSha(Buffer.from(fullDomainName)),
    ).toString('hex');

    const saltedDomainHashBuffer = doubleSha(Buffer.concat([
      bs58.decode(preorderSalt),
      Buffer.from(nameHash, 'hex'),
    ]));

    const saltedDomainHash = createMultiHash(
