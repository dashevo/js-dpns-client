...
function registerMethodFactory(dapiClient, dpp, wallet) {
  ...
  async function registerMethod(name, regTxId) {
    ...
    const saltedDomainHash = multihash(
    ...
    // 2. Create and send preporder transaction
    const {
      transaction: preorderTransaction,
      stPacket: preorderSTPacket,
    } = createDocumentTransactionData(
      preorderDocument,
      prevSTHash,
    );
    ...
    const nameHash = multihash(Buffer.from(name)).toString('hex');
